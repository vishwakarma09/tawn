...
class ASPBrowser {
     ...
     * @return array|string
     */
    function createASPPostParamsForRadio($dom, array $params) {
        $postData = $dom->find('input[!=checkbox],select,textarea');
        $postFields = array();
				$processedField = array();
        foreach($postData as $d) {
            $name = $d->name;
						if(in_array($name, $processedField)) continue;	// to make sure that only unique form elements are sent. checkboxes
						else array_push($processedField, $name);
            if(trim($name) == '' || in_array($name, $this->exclude)) continue;
            $value = isset($params[$name]) ? $params[$name] : $d->value;
            $postFields[] = rawurlencode($name).'='.rawurlencode($value);
        }
    ...
    function doPostRequest($url, array $params) {
      //  $post = $this->createASPPostParams($this->dom, $params);
        $post = $this->createASPPostParamsForRadio($this->dom, $params);
